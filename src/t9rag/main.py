...
from .trainer import process_documents_with_questions
from .vector_store import DocumentDict, VectorStore

BASEDIR = Path(__file__).parent.resolve()


class ProcessDocumentsOptions(TypedDict):
    config: str
    directory: Path
    db_directory: str
    chunk_size: int
...
@click.pass_context
def process_documents(ctx: click.Context, **options: Unpack[ProcessDocumentsOptions]) -> None:
    if options.get("config"):
        # If config is passed in, the values in config take precedence over CLI options
        # unless an option is explicitly provided
        config, options = read_config(ctx, options, "train")
    else:
        config = {"prompts": {}}
    ...
    if llm is None:
        ...
        return
    _ = process_documents_with_questions(
        options["directory"],
        options["chunk_size"],
        options["chunk_overlap"],
        llm,
        get_prompt(options["prompt"], config or {}),
    )
...
cli.add_command(process_documents, "train")
